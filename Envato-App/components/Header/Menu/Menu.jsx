  ...
  return (
    <>
      <div className="sticky top-14 bg-white h-18 ">
        <header className="hidden md:flex justify-around items-center-safe h-18 px-8">
          <div className="flex justify-between items-center ">
            <img src="./companyLogo.svg" alt="T" className="h-full w-20 hover:scale-105"/>
          </div>
          <nav className=" flex text-black space-x-4 ">
            {menuItems.map((item, index) => (
              <div key={index} className="relative group">
                <a href={item.href || "#"} className="hover:text-[#00a3c8]">
                  {item.name}
                </a>
                {item.subPages && (
          ...
          {/* Hamburger Menu Icon */}
          <i className="bx bx-menu text-black md:hidden xl:hidden block cursor-pointer text-3xl"></i>
        </header>
      </div>
    </>
  );
